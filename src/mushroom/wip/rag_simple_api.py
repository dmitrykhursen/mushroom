...
class WikiApi:
    ...
    def search(self, search_string):
        ...
        for api_result in api_results:
            cleaned_extract = re.sub(r'<span .*?>(.*?)</span>', r'\1', api_result['excerpt'])
...
class FactSupportExtractor:
    ...
    def __init__(self, mistral_api_key=None, wiki_api=WikiApi('en'), mistral_model="mistral-large-latest"):
        self._mistral = Mistral(api_key=mistral_api_key) if mistral_api_key is not None else None
...
if __name__ == '__main__':
    ...
    found_support = support_finder.find_support(['Xiong Ai was a Chinese warlord', 'all insects and arachnids'])
    print(found_support)
